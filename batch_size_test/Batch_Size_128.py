...
checkpoint = keras.callbacks.ModelCheckpoint(
    filepath='checkpoint/best_model_effnet_Batch_Size_128',
    save_best_only= True,
    monitor='val_accuracy',
    mode='max'
)

model_run = model.fit(train_ds, validation_data=test_ds, epochs=25, callbacks=[stopper,checkpoint])

plot_hist(model_run)
plt.savefig('Accuracy Curve_Base_Model_Batch_Size_128')
plt.show()

plot_hist_loss(model_run)
plt.savefig('Loss Curve_Base_Model_Batch_Size_128')
plt.show()
# model = tf.keras.models.load_model(filepath='checkpoint/best_model_effnet_Batch_Size_128')
